...
export default function Users({ auth, users }) {
    ...
    // Fungsi untuk menutup dropdown filter kalau diklik di luar area
    useEffect(() => {
    ...
    const filteredUsers = dummyUsers.filter(user => {
        // 1. Cek Pencarian (Berdasarkan Nama, NIP, atau Email)
        const matchesSearch =
                ...
                <div className="flex-1 min-w-0 flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="bg-transparent border-t border-b sm:border border-gray-100 sm:bg-white sm:rounded-[24px] sm:shadow-sm overflow-hidden flex-1 flex flex-col">
                        ...
                        <div className="p-4 sm:p-5 flex flex-col sm:flex-row items-center gap-4 bg-white border-b border-gray-100">
                            ...
                            {/* Tombol Add Filter dengan Dropdown */}
                            <div className="relative w-full sm:w-auto" ref={filterRef}>
                                <button
                                    onClick={() => setIsFilterOpen(!isFilterOpen)}
                                    className={`flex items-center justify-between sm:justify-start gap-2 text-sm font-medium transition-all px-3 py-2 rounded-xl border w-full sm:w-auto ${statusFilter !== 'Semua' ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-transparent text-gray-500 hover:text-gray-800 hover:bg-gray-50'}`}
                                >
                                    ...
                                    <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl z-50 py-2 animate-in fade-in slide-in-from-top-2">
                                        ...
                                        {['Semua', 'Aktif', 'Nonaktif', 'Cuti'].map(status => (
                                            <button
                                                ...
                                                className={`w-full text-left px-4 py-2 text-sm transition-colors flex items-center justify-between ${statusFilter === status ? 'bg-blue-50 text-blue-700 font-bold' : 'text-gray-700 hover:bg-gray-50'}`}
                                            >
                                                {status}
                            ...
                            <div className="relative w-full flex-1 group">
                                ...
                                <input
                                    ...
                                    className="w-full bg-transparent border-none pl-8 pr-4 py-2 text-sm text-gray-800 outline-none placeholder-gray-400 focus:ring-0"
                                />
                                        ...
                                        filteredUsers.map((u) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-5 sm:px-6 py-3.5"><span className={`text-[13px] ${isSelected ? 'text-white' : 'text-gray-500'}`}>{u.phone}</span></td>
                                                </tr>
                                            );
                                        })
                                    ) : (
                                        <tr>
                                            <td colSpan="5" className="px-6 py-20 text-center">
                                                <div className="flex flex-col items-center justify-center text-gray-400">
                                                    ...
                                                    <p className="text-xs mt-1">Coba gunakan kata kunci atau filter lain.</p>
                                                    <button
                                                        onClick={() => { setSearchQuery(''); setStatusFilter('Semua'); }}
                                                        className="mt-4 text-xs font-bold text-blue-600 hover:text-blue-800 bg-blue-50 px-4 py-2 rounded-lg transition-colors"
                                                    >
                                                        Reset Pencarian
                                                    </button>
                                                </div>
                ...
                <div className="w-full xl:w-[320px] shrink-0 animate-in fade-in slide-in-from-right-8 duration-500 delay-150 fill-mode-both">
                    ...
                    <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 p-8 flex flex-col items-center sticky top-24 transition-all">
                        ...
                        <div className="flex gap-3 mb-8 w-full justify-center">
                            <button className="w-10 h-10 rounded-xl bg-gray-50 border border-gray-200 flex items-center justify-center text-gray-500 hover:text-[#4B70F5] hover:bg-blue-50 transition-colors shadow-sm" title="Hubungi">
